...
// QueryOrderer chains the current query on the "orderer" edge.
func (uq *UserQuery) QueryOrderer() *OrderQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOrderer: uq.withOrderer.Clone(),
...
// WithOrderer tells the query-builder to eager-load the nodes that are connected to
// the "orderer" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithOrderer(opts ...func(*OrderQuery)) *UserQuery {
	query := (&OrderClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withOrderer = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withOrderer; query != nil {
		if err := uq.loadOrderer(ctx, query, nodes,
			func(n *User) { n.Edges.Orderer = []*Order{} },
			func(n *User, e *Order) { n.Edges.Orderer = append(n.Edges.Orderer, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadOrderer(ctx context.Context, query *OrderQuery, nodes []*User, init func(*User), assign func(*User, *Order)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.OrdererColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
