...
type OrderQuery struct {
	...
	withUser       *UserQuery
	withValidation *OrderValidationQuery
	withFKs        bool
...
func (oq *OrderQuery) QueryValidation() *OrderValidationQuery {
	query := (&OrderValidationClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withValidation: oq.withValidation.Clone(),
...
func (oq *OrderQuery) WithValidation(opts ...func(*OrderValidationQuery)) *OrderQuery {
	query := (&OrderValidationClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withValidation = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withValidation; query != nil {
		if err := oq.loadValidation(ctx, query, nodes,
			func(n *Order) { n.Edges.Validation = []*OrderValidation{} },
			func(n *Order, e *OrderValidation) { n.Edges.Validation = append(n.Edges.Validation, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadValidation(ctx context.Context, query *OrderValidationQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderValidation)) error {
	...
	query.withFKs = true
	query.Where(predicate.OrderValidation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.ValidationColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.order_validation_order
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_validation_order" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_validation_order" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
